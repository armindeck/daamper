...
if(!isset($TIPO)){
	mensajeSpan(['bg'=>'yellow', 'co'=>'#000','text'=>'No tiene acceso al archivo.','ruta'=>"{$Web['directorio']}auth/iniciar{$Web['config']['php']}"]);
}
...
if(!isset($_POST['resultado']) || !isset($_POST['resultado_verificar'])){
	mensajeSpan(['bg'=>'red','text'=>'Por favor no modifique los campos ocultos.','ruta'=>"{$Web['directorio']}auth/{$DIRECCION}"]);
}
...
if($TIPO=='configuracion'){
	if(isset($_POST['tipo'])){
		...
		mensajeSpan(['bg'=>'red','text'=>'Oh! parece que algo salio mal.','ruta'=>"{$Web['directorio']}auth/{$DIRECCION}"]);
	}
}
...
if(md5('R+_'. SCRIPTS->normalizar2($_POST['resultado']) .'-W') != SCRIPTS->normalizar2($_POST['resultado_verificar'])){
	...
	mensajeSpan(['bg'=>'red','text'=>'La suma es incorrecta, intentelo de nuevo.','ruta'=>"{$Web['directorio']}auth/{$DIRECCION}",'tmpForm'=>$pos]);
}

if($TIPO=='no_cambiar_contrasena'){
	if(isset($_SESSION['id'])){
		unset($_SESSION['cambiar_contrasena']);
		mensajeSpan(['bg'=>'red','text'=>'Se recomienda cambiar la contraseña en el caso de olvidarla.','ruta'=>"{$Web['directorio']}auth/iniciar{$Web['config']['php']}"]);
	}
}

foreach ($list['datos_procesa'] as $key => $value) {
	if(!isset($_POST[$value])){
		mensajeSpan(['bg'=>'red','text'=>'Por favor llene los campos necesarios.','ruta'=>"{$Web['directorio']}auth/{$DIRECCION}",'tmpForm'=>$pos]); break;
	}
}
...
if(!$verificar) {
	mensajeSpan(['bg'=>'red','text'=>'Por favor llene los campos con los datos necesarios.','ruta'=>"{$Web['directorio']}auth/{$DIRECCION}",'tmpForm'=>$pos]);
}

if($TIPO=='registrar'){
	if($pos['contrasena'] != $pos['contrasena_confirmar']){
		mensajeSpan(['bg'=>'red','text'=>'La contraseña es diferente en los campos.','ruta'=>"{$Web['directorio']}auth/{$DIRECCION}",'tmpForm'=>$pos]);
	}
}

if($TIPO=='cambiar_contrasena'){
	if($pos['nueva_contrasena'] != $pos['nueva_contrasena_confirmar']){
		mensajeSpan(['bg'=>'red','text'=>'La contraseña es diferente en los campos.','ruta'=>"{$Web['directorio']}auth/{$DIRECCION}"]);
	}
}
...
if($TIPO!='registrar'){
	if(!isset($usu)) {
		mensajeSpan(['bg'=>'red','text'=>'Parece que todavia no existe ningún usuario registrado o no existe la db.','ruta'=>"{$Web['directorio']}auth/{$DIRECCION}",'tmpForm'=>$pos]);
	}
}
	...
	foreach ($usu as $key => $value) {
		if(strtolower($pos['usuario']) == strtolower($value['usuario']) && md5($pos['contrasena']) == $value['contrasena']){
			if($value['estado']!='publico'){
				mensajeSpan(['bg'=>'red','text'=>'Este perfil esta suspendido o fue eliminado.','ruta'=>"{$Web['directorio']}auth/{$DIRECCION}",'tmpForm'=>$pos]);
			}
			$id_usuario = $value['id'];
			break;
		}
		...
		foreach ($usu as $key => $value) {
			if($pos['email'] == $value['email']){
				if($_SESSION['id'] != $value['id']){
					mensajeSpan(['bg'=>'red','text'=>'El e-mail ingresado ya fue usado.','ruta'=>"{$Web['directorio']}auth/{$DIRECCION}",'tmpForm'=>$pos]);
				}
			}
		}
	}
...
if($TIPO=='cambiar_contrasena'){
	if(!isset($_SESSION['cambiar_contrasena'])){
		if(md5($pos['contrasena']) != $usu[$_SESSION['id']]['contrasena']){
			mensajeSpan(['bg'=>'red','text'=>'La contraseña es incorrecta.','ruta'=>"{$Web['directorio']}auth/{$DIRECCION}"]);
		}
	}
	if(md5($pos['nueva_contrasena']) == $usu[$_SESSION['id']]['contrasena']){
		mensajeSpan(['bg'=>'red','text'=>'Por favor use otra contraseña.','ruta'=>"{$Web['directorio']}auth/{$DIRECCION}"]);
	}
...
if($TIPO == 'configuracion'){
	if(md5($pos['contrasena']) != $usu[$_SESSION['id']]['contrasena']){
		mensajeSpan(['bg'=>'red','text'=>'La contraseña es incorrecta.','ruta'=>"{$Web['directorio']}auth/{$DIRECCION}",'tmpForm'=>$pos]);
	} else {
	...
	if($_POST['tipo']=='eliminar_cuenta'){
		if($pos['confirmar']!='Si eliminar'){
			mensajeSpan(['bg'=>'red','text'=>'Tiene que confirmar la eliminación para continuar.','ruta'=>"{$Web['directorio']}auth/{$DIRECCION}",'tmpForm'=>$pos]);
		}
...
if($TIPO=='iniciar'){
	if(!isset($id_usuario)) {
		mensajeSpan(['bg'=>'red','text'=>'El usuario o la contraseña es incorrecta.','ruta'=>"{$Web['directorio']}auth/{$DIRECCION}",'tmpForm'=>$pos]);
	}
...
if($TIPO=='olvide_contrasena'){
	if(!isset($id_usuario)) {
		mensajeSpan(['bg'=>'red','text'=>'El usuario o el e-mail es incorrecto, esta suspendido o fue eliminado.','ruta'=>"{$Web['directorio']}auth/{$DIRECCION}",'tmpForm'=>$pos]);
	}
}
	...
	if(!isset($usu)){ $usu_idr = 1; $guardar .= $guardar_ini; } else {
		foreach ($usu as $key => $value) {
			if(strtolower($pos['usuario']) == strtolower($value['usuario'])){
				if($value['estado']=='publico'){
					mensajeSpan(['bg'=>'red','text'=>'El usuario ya esta en uso.','ruta'=>"{$Web['directorio']}auth/{$DIRECCION}",'tmpForm'=>$pos]); break;
				} else {
					mensajeSpan(['bg'=>'red','text'=>'El usuario que selecciono fue suspendido o eliminado.','ruta'=>"{$Web['directorio']}auth/{$DIRECCION}",'tmpForm'=>$pos]); break;
				}
			}
			if($pos['email'] == $value['email']){
				if($value['estado']=='publico'){
					mensajeSpan(['bg'=>'red','text'=>'El e-mail ya esta en uso.','ruta'=>"{$Web['directorio']}auth/{$DIRECCION}",'tmpForm'=>$pos]); break;
				} else {
					mensajeSpan(['bg'=>'red','text'=>'El e-mail que selecciono fue suspendido o eliminado.','ruta'=>"{$Web['directorio']}auth/{$DIRECCION}",'tmpForm'=>$pos]); break;
				}
			}
		}
		$usu_idr = count($usu)+1;
...
# DIRECCIONES EXITO
if($TIPO=='registrar'){
	mensajeSpan(['bg'=>'green','text'=>'El usuario se registro exitosamente!','ruta'=>"{$Web['directorio']}auth/iniciar{$Web['config']['php']}",'tmpForm'=>['usuario'=>$pos['usuario']]]);
}

if($TIPO=='iniciar'){
	mensajeSpan(['bg'=>'green','text'=>'Bienvenid@ de nuevo!','ruta'=>"{$Web['directorio']}p/{$usu[$id_usuario]['usuario']}{$Web['config']['php']}"]);
}
if($TIPO=='cambiar_contrasena'){
	mensajeSpan(['bg'=>'green','text'=>'La contraseña fue cambiada exitosamente!','ruta'=>"{$Web['directorio']}p/{$usu[$_SESSION['id']]['usuario']}{$Web['config']['php']}"]);
}
if($TIPO=='olvide_contrasena'){
	$_SESSION['cambiar_contrasena'] = true;
	mensajeSpan(['bg'=>'green','text'=>'Quieres cambiar la contraseña?','ruta'=>"{$Web['directorio']}p/{$usu[$id_usuario]['usuario']}{$Web['config']['php']}"]);
}

if($TIPO=='configuracion'){
	mensajeSpan(['bg'=>'green','text'=>'Los datos se actualizaron!','ruta'=>"{$Web['directorio']}auth/{$DIRECCION}"]);
}
?>