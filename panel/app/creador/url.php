...
if (isset($_POST['refrescar']) && !empty($_POST['refrescar'])) {
	...
	mensajeSpan(['bg' => 'rgb(64,255,255)', 'co' => '#000', 'text' => (LANGUAJE['global']['refreshed'][CONFIG['languaje']]), 'ruta' => $ruta_get, 'tmpForm' => $_POST]);
}
...
if (
	...
	if ($Creador['get_tipo'] == 'publicacion' || $Creador['get_tipo'] == 'borrador') {
		...
		if (!file_exists($ruta_archivo)) {
			mensajeSpan(['bg' => 'red', 'text' => (LANGUAJE['global']['file-no-exists'][CONFIG['languaje']][0]). ' <strong>' . $Creador['get_archivo'] . '</strong> ' .(LANGUAJE['global']['file-no-exists'][CONFIG['languaje']][1]), 'ruta' => "?ap=creador"]);
		}
		require $ruta_archivo;
		if (!isset($ACR['creador'])) {
			mensajeSpan(['bg' => 'red', 'text' => (LANGUAJE['alert']['file-exists-no-data-or-incomplete'][CONFIG['languaje']]), 'ruta' => "?ap=creador"]);
		}
		$Creador['lista_datos_necesarios'] = ['creador', 'db_ruta'];
		foreach ($Creador['lista_datos_necesarios'] as $key => $value) {
			if (!isset($ACR[$value])) {
				mensajeSpan(['bg' => 'red', 'text' => (LANGUAJE['alert']['file-no-data-creator-or-ruta'][CONFIG['languaje']]), 'ruta' => "?ap=creador"]);
			}
		}
...
if (isset($_GET['actualizar-entradas']) && isset($_GET['cantidad-entradas']) && !empty($_GET['cantidad-entradas'])) {
	...
	if (!$funciona) {
		mensajeSpan(['bg' => 'red', 'text' => (LANGUAJE['global']['data-no-save'][CONFIG['languaje']]), 'ruta' => "panel.php?ap=creador"]);
	}
	mensajeSpan(['bg' => 'green', 'text' => (LANGUAJE['global']['data-save'][CONFIG['languaje']]), 'ruta' => "panel.php?ap=creador"]);
}
