...
if (empty($AC_FORMULARIO['referencia'])) {
	if(!$Post['existe_el_archivo_creador']){
		mensajeSpan(['bg'=>'red','text'=>Language(['creador', 'please-select-a-reference'], 'dashboard'),'ruta'=>"../panel.php?ap=creador&creador={$AC_FORMULARIO['creador']}"]);
	} else {
		mensajeSpan(['bg'=>'red','text'=>Language(['creador', 'please-select-a-reference'], 'dashboard'),'ruta'=>"../panel.php?ap=creador&creador={$ACR_FORMULARIO['creador']}&tipo={$ACR_FORMULARIO['pubo']}&archivo={$ACR_FORMULARIO['db_archivo']}"]);
	}
}
...
if(file_exists($Web['directorio'].'app/database/publicaciones/'.$AC_FORMULARIO['referencia'])){
	...
	if(!$Post['existe_el_archivo_creador']){
		mensajeSpan(['bg'=>'red','text'=>Language(['creador', 'reference-does-not-exist'], 'dashboard'),'ruta'=>"../panel.php?ap=creador&creador={$AC_FORMULARIO['creador']}"]);
	} else {
		mensajeSpan(['bg'=>'red','text'=>Language(['creador', 'reference-does-not-exist'], 'dashboard'),'ruta'=>"../panel.php?ap=creador&creador={$ACR_FORMULARIO['creador']}&tipo={$ACR_FORMULARIO['pubo']}&archivo={$ACR_FORMULARIO['db_archivo']}"]);
	}
}
