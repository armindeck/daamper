...
<section class="flex-evenly lista-ver" style="margin: 4px;">
	<section class="flex-column">
		<header style="margin-bottom: 8px;">
			<small class="ruta">
				<a href="<?= $Web['directorio']; ?>"><i class="fas fa-inicio"></i> Inicio</a> >
			...
			<header class="opciones-botones">
				<?php for ($i = 1; $i <= ($AX['cantidad_servidor_stream'] ?? 0); $i++): ?>
					<div class="opcion">
						...
						<a href="<?= $AX['servidor_stream_'.$i.'_enlace'] ?>" rel="nofollow" target="servidor-stream">Opción <?= $i ?></a>
					</div>
				<?php endfor; ?>
			</header>
					...
					<?php function BotonMinMax ($Mm = false) { global $Web, $AX;
						...
						if (file_exists($file)) { ?>
							<a class="boton t-14" href="<?= str_replace(".php", "", $file) . ($Web['config']['php'] ?? '') ?>">
								<?= !$Mm ? '<i class="fas fa-chevron-circle-left"></i> Anterior' : 'Siguiente <i class="fas fa-chevron-circle-right"></i>' ?>
							</a>
						<?php }
					} ?>
				<?= BotonMinMax() ?>
				<a class="boton t-14" href="<?= $Web['directorio'].$AX['ruta_referencia'].SCRIPTS->quitarEPHP($AX['archivo_referencia']).$Web['config']['php'] ?>"><i class="fas fa-th-list"></i></a>
				<?= BotonMinMax(true) ?>
				</div>
				<input type="checkbox" id="check-boton" hidden>
				<label class="check-boton" for="check-boton">
					<a class="boton t-14"><i class="fas fa-download"></i> Descargar</a>
				</label>
				<section class="con check-div" style="width: 100%;">
					<p><i class="fas fa-download"></i> Descargas</p><hr>
					<?php for ($i = 1; $i <= ($AX['cantidad_servidor_descarga'] ?? 0); $i++): ?>
						<div class="flex-between">
							<p><?= $AX['servidor_descarga_'.$i] ?></p>
							<section>
								<a class="boton t-14" href="<?= $AX['servidor_descarga_'.$i.'_enlace_acortado'] ?>" rel="nofollow" target="_blank"><i class="fas fa-arrow-alt-circle-down"></i> Descargar</a>
								<?php if(isset($_SESSION['rol']) && in_array(strtolower($_SESSION['rol']), ['administrador','ceo founder'])): ?>
									<a class="boton-2 t-14" href="<?= $AX['servidor_descarga_'.$i.'_enlace'] ?>" rel="nofollow" target="_blank"><i class="fas fa-bolt"></i> Directa</a>
								<?php endif; ?>
							</section>
						</div>
					<?php endfor; ?>
	...
	<article>
		<section class="con">
			<a href="<?= WEBSITE->creador_enlace ?>">
				<i class="fas fa-star"></i> <?= WEBSITE->creador_nombre_web ?> <i class="fas fa-star"></i>
			</a>
		</section>
		<?= PlantillaComandos('[Return=Iframe height="890px" src="'.(WEBSITE->creador_enlace).'/web/index.php" get="view~main&tema~'.(isset($_SESSION['tmp']['tema']) ? $_SESSION['tmp']['tema'] : 'blue-aero').'"]', 0, 0) ?>
	</article>
</section>
<?php endif; ?>
