<?= pSelectArchivos(['style' => 'width: 100%;', 'name'=>'referencia','referencia' => ['anime','hentai'],'texto'=>Language('reference'),'ruta'=>$Web['directorio'].'app/database/publicaciones/pu_','tipo_archivos'=>'php','value-devuelve'=>'basename']) ?>
<?= pInput(['name'=>'stream_default','type'=>'url','placeholder'=>Language('stream-default'),'texto'=>Language('stream-default'),'required'=>true]) ?>

<?php $lista_servidores['stream'] = Database('creator/list')['server-streams'];
$lista_servidores['descarga'] = Database('creator/list')['server-downloads'];
