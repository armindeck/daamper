...
if(isset($_POST['crear_carpeta_boton']) && !empty($_POST['crear_carpeta_boton'])){
	if(!file_exists($Panel['ap_directorio_dir'].SCRIPTS->normalizar($_POST['carpeta']))){
		...
		mensajeSpan(['bg'=>'green','text'=>(LANGUAJE['dashboard']['directorio']['directory-created'][CONFIG['languaje']]),'ruta'=>"?ap=directorio&dir={$Panel['ap_directorio_dir']}"]);
	} else {
		mensajeSpan(['bg'=>'red','text'=>(LANGUAJE['dashboard']['directorio']['directory-exists'][CONFIG['languaje']]),'ruta'=>"?ap=directorio&dir={$Panel['ap_directorio_dir']}"]);
	}
}

if(isset($_POST['eliminar_carpeta_boton']) && !empty($_POST['eliminar_carpeta_boton'])){
	if($_POST['confirmar'] != 'Si'){
		mensajeSpan(['bg'=>'red','text'=>(LANGUAJE['dashboard']['directorio']['confirm-delete'][CONFIG['languaje']]),'ruta'=>"?ap=directorio&dir={$Panel['ap_directorio_dir']}"]);
	}
	if(file_exists($Panel['ap_directorio_dir'].SCRIPTS->normalizar($_POST['carpeta']))){
		...
		mensajeSpan(['bg'=>'green','text'=>(LANGUAJE['dashboard']['directorio']['deleted'][CONFIG['languaje']]).': <i class="fas fa-folder"></i> '.$eliminados_carpeta.' <i class="fas fa-file-code"></i> '.$eliminados_archivos,'ruta'=>"?ap=directorio&dir={$Panel['ap_directorio_dir']}"]);
	} else {
		mensajeSpan(['bg'=>'red','text'=>(LANGUAJE['dashboard']['directorio']['directory-no-exists'][CONFIG['languaje']]),'ruta'=>"?ap=directorio&dir={$Panel['ap_directorio_dir']}"]);
	}
}
