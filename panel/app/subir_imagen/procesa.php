...
if(isset($_FILES['imagen']) && $_FILES['imagen']['error'] !== UPLOAD_ERR_NO_FILE){
        ...
        if($file_error>0){
            mensajeSpan(['bg'=>'red','text'=>(Language(['subir-imagen', 'error-upload-image'], 'dashboard')),'ruta'=>"../panel.php?ap=$Apartado"]);
         } else {
            if($file_tipo=='image/jpg' || $file_tipo=='image/jpeg' || $file_tipo=='image/png' || $file_tipo=='image/gif'){
                ...
                if($file_tamano>1048576){
            		mensajeSpan(['bg'=>'red','text'=>(Language(['subir-imagen', 'image-max-size'], 'dashboard')),'ruta'=>"../panel.php?ap=$Apartado"]);
                }{
                    ...
                    mensajeSpan(['bg'=>'green','text'=>(Language(['subir-imagen', 'uploaded-image'], 'dashboard')).': <a target="_blank" href="../assets/img/'.$nombre_final.'">'.(Language('show')).' <i class="fas fa-external-link-alt"></i></a>','ruta'=>"../panel.php?ap=$Apartado"]);
                }
            } else {
            	mensajeSpan(['bg'=>'red','text'=>(Language(['subir-imagen', 'format-accept'], 'dashboard')),'ruta'=>"../panel.php?ap=$Apartado"]);
            }
        }
    } else {
        mensajeSpan(['bg'=>'red','text'=>(Language(['subir-imagen', 'error-uploading'], 'dashboard')),'ruta'=>"../panel.php?ap=$Apartado"]);
    }
