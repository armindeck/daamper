...
if ($Web['ruta_completa'] != '../../admin/process/creator.php' && $Web['ruta'] == 'report.php') {
    ...
    if (isset($_GET['r']) && !empty($_GET['r'])) {
        $get['r'] = Daamper::$scripts->normalizar2($_GET['r']);
        $id = $get['r'];
        $comentarios = [];
        if (file_exists(Daamper::$data->CommentRoute())){ $comentarios = Daamper::$data->Comment(); }
        if (file_exists(Daamper::$data->CommentRoute(true))){ $comentarios = Daamper::$data->CommentAll(); }
        if (count($comentarios) <= 0) { ?>
            <section class="con" style="text-align: center;"><?= Language('no-comments', 'form') ?></section>
        <?php }
        for ($i = 1; $i <= count($comentarios); $i++) {
            if (Daamper::$scripts->SimpleToken($i) == $id) { $id = $i; $encontro = true; break; }
        }
        if (!isset($encontro)) { ?>
        ...
        if ($comentarios[$id]['estado'] == 'publico') { ?>
            <form method="post" action="./process/report.php">
                ...
                <input type="hidden" name="resultado_verificar" value="<?= Daamper::$scripts->SimpleToken($suma["c"]); ?>"><hr>
