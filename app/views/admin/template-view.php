<?php $Apartado='template'; ?>
<form method="post" action="process/actions.php" style="max-width: 99%;">
	<section class="panel">
		<section class="form" style="margin-bottom: 0;">
			<section class="flex-between">
				...
				<section>
					...
					pEnlace(['texto'=>Language('show'),'icono'=>'fas fa-external-link-alt','class'=>'boton-2 boton-mini','target'=>'_blank','href'=>'?ap=directory&dir=../app/scripts/template/']);
					?>
				</section>
			</section>
			<details>
				...
				<section class="flex-column t-14">
					<?php $ruta_plantillas = glob('../database/template/*.json');
						foreach ($ruta_plantillas as $plantilla) {
							if (substr(basename($plantilla), 0, 4) != 'scr-' && !in_array(basename($plantilla), ["template.json", "scr-template.json"])) {
								echo '<a href="?ap=template&plantilla='.basename($plantilla).'&accion=mostrar">'.str_replace([".php", ".json"], "", basename($plantilla)).'</a>';
							}
						}
					?>
				</section>
			...
			<details>
				...
				<section class="flex-column">
					...
					<details open>
						...
						<section class="flex-between">
							<a class="boton-2 boton-mini" href="?ap=template&accion=restaurar"><?= Language('restore') ?></a>
						</section>
					</details>
				</section>
			</details>
