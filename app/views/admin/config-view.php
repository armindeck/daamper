...
<section class="panel">
	...
	<form method="post" class="label-a" action="process/actions.php">
		...
		pSelect(['name' => 'language', 'label' => true, 'value' => (isset($_SESSION['tmp']['language']) ? $_SESSION['tmp']['language'] : CONFIG['language']), 'texto' => Language('language'), 'option' => DATA->Read('config/language')['global']['languages-options'][(isset($_SESSION['tmp']['language']) ? $_SESSION['tmp']['language'] : CONFIG['language'])]]).
		...
		<details open>
			...
			<section>
			<?php foreach (DATA->Config("default")["api"]["show"] as $key => $value) { ?>
				<details open>
			...
			<details open>
				...
				<section>
					<?php foreach (DATA->Config("default")["api"]["auto"] as $value) {
						echo pCheckboxBotonActivoDesaptivo($Web, $Apartado, [
