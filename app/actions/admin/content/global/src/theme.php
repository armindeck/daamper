<?php if (isset($_GET['tema']) && !empty($_GET['tema'])) {
	if(file_exists($Web["directorio"] . 'database/theme/' . $_GET['tema'])):
		$Web['theme']['styles'] = DATA->Read('theme/' . SCRIPTS->normalizar($_GET['tema']));
		if(!empty($Web['theme']['styles'])): ?>
		<section class="form flex-between" style="margin-top: 0px; margin-bottom: 0;">
			<a href="?ap=theme&tema=<?= SCRIPTS->normalizar($_GET['tema']) ?>&accion=Utilizar" class="boton"><?= Language('use') ?></a>
			<a href="?ap=theme&tema=<?= SCRIPTS->normalizar($_GET['tema']) ?>&accion=Normalizar" class="boton"><?= Language('normalize') ?></a>
			<a href="?ap=theme&tema=<?= SCRIPTS->normalizar($_GET['tema']) ?>&accion=Eliminar" class="boton2"><?= Language('delete') ?></a>
		</section>
		<?php endif; ?>
	...
	<?= isset($_GET['accion']) && in_array($_GET['accion'], ['eliminar', 'normalizar', 'utilizar']) ?
		'<section class="form" style="text-align: center; margin-top: 0; margin-bottom: 0; color: white; background-color: '. ($archivo_tema_confirmar ? 'green' : 'red') .';"><p>' . (
			$archivo_tema_confirmar ? (
				$_GET['accion'] == 'eliminar' ? Language('file-delete', 'global', ['value' => SCRIPTS->normalizar($_GET['tema'])]) :
