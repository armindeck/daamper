<?php function PlantillaComandosReemplazar ($comando, $comando_valor, $comando_atributos, $Contenedor, $Elemento) { global $Web;
	if (in_array($comando, ['Form', 'Return', 'Post'])) {
		...
		if (in_array($comando_valor, ['Input','InputEnlace'])) {
			$comando_atributos['value'] = $Web['plantilla']['scr'][$comando_atributos['name']] ?? '';
		}
	...
	if ($comando == 'Form') {
		switch ($comando_valor) {
			case 'Input':
				...
				break;
			case 'InputEnlace':
				return pInputEnlace($Web, ['plantilla','scr'], ['name' => $comando_atributos['name']], $Contenedor, $Elemento);
				break;
	...
	if ($comando == 'Return') {
		...
		switch ($comando_valor) {
			...
			case 'Visitas': return Database('other/count')['total'] ?? 0; break;
			case 'Iframe':
			...
			case 'FileReturnVersion':
				...
				break;
			case 'InputEnlace':
				return EnlaceExtructura($Web, ['plantilla','scr'], ['name' => $comando_atributos['name'], 'solo' => $comando_atributos['solo'] ?? '', 'class' => $comando_atributos['class'] ?? ''], isset($comando_atributos['contenedor']) ? $comando_atributos['contenedor'] : $Contenedor, isset($comando_atributos['elemento']) ? $comando_atributos['elemento'] : $Elemento);
				break;
            case 'InputEnlaceSesion':
				return isset($_SESSION['id']) ? EnlaceExtructura($Web, ['plantilla','scr'], ['name' => $comando_atributos['name'], 'solo' => $comando_atributos['solo'] ?? '', 'class' => $comando_atributos['class'] ?? ''], isset($comando_atributos['contenedor']) ? $comando_atributos['contenedor'] : $Contenedor, isset($comando_atributos['elemento']) ? $comando_atributos['elemento'] : $Elemento) : '<i hidden></i>';
				break;
            case 'InputEnlaceNoSesion':
				return !isset($_SESSION['id']) ? EnlaceExtructura($Web, ['plantilla','scr'], ['name' => $comando_atributos['name'], 'solo' => $comando_atributos['solo'] ?? '', 'class' => $comando_atributos['class'] ?? ''], isset($comando_atributos['contenedor']) ? $comando_atributos['contenedor'] : $Contenedor, isset($comando_atributos['elemento']) ? $comando_atributos['elemento'] : $Elemento) : '<i hidden></i>';
				break;
			case 'InputEnlaceNoPanel':
				return $Web['ruta_completa'] != '../panel/panel.php' ? EnlaceExtructura($Web, ['plantilla','scr'], ['name' => $comando_atributos['name'], 'solo' => $comando_atributos['solo'] ?? '', 'class' => $comando_atributos['class'] ?? ''], isset($comando_atributos['contenedor']) ? $comando_atributos['contenedor'] : $Contenedor, isset($comando_atributos['elemento']) ? $comando_atributos['elemento'] : $Elemento) : '<i hidden></i>';
				break;
		}
